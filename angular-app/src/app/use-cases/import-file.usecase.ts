...
import { XlsxReaderService } from '../application/services/xlsx-reader.service';
...
export class ImportFileUseCase {
  ...
  async importFile(file: File): Promise<ImportData[]> {
    ...
    const bbvaData = mapCsvRowsToBBVAImportData(csvRows); // Only maps date, description, amount
